      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {products.map((product) => (
          ...
          >
            ...
            {selectedToppings[product._id]?.length > 0 && (
              <p className="text-sm text-green-600">
                Selected:{" "}
                {selectedToppings[product._id].map((t) => t.name).join(", ")}
              </p>
            )}
            ...
            <button
              ...
              className="mt-3 bg-white border border-yellow-500 text-black px-3 py-1 rounded hover:bg-yellow-50"
            >
              Add to Cart
            </button>
          </div>
